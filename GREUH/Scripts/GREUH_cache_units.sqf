...
while { true } do {
	...
	_scanned_units = [ allUnits, { ( alive _x ) && ( side group _x == WEST ) } ] call BIS_fnc_conditionalSelect;
	_scanned_units = [ _scanned_units, { _x distance player < nametags_distance } ] call BIS_fnc_conditionalSelect;
