...
FAR_HandleDamage_EH =
{
	...
	if ( _selectionName in [ "body", "spine1", "spine2", "spine3", "pelvis", "" ] ) then {
		...
		if ( _vestobject != ""  ) then {
			_vestarmor = ( (configFile >> "CfgWeapons" >> _vestobject >> "ItemInfo" >> "armor" ) call BIS_fnc_GetCfgData ) / 60.0;
			...
			if ( _vestarmor > 0.8 ) then { _vestarmor = 0.8; };
		};
	};
	...
	if ( _selectionName in [ "head", "neck", "" ] ) then {
		...
		if ( _helmetobject != ""  ) then {
			_helmetarmor = ( (configFile >> "CfgWeapons" >> _helmetobject >> "ItemInfo" >> "armor" ) call BIS_fnc_GetCfgData ) * 0.15;
