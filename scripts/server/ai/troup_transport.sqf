...
_spawn_distance = 1000;
_unload_distance = 500;
sleep 1;
_initial_crewcount = count crew _troup_transport;

waitUntil { sleep 0.2; !(alive _troup_transport) || !(alive (driver _troup_transport)) || (((_troup_transport distance _dat_objective) < _spawn_distance) && (!(surfaceIsWater (getpos _troup_transport)))) };
