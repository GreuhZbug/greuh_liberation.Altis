...
while { endgame == 0 } do {
	...
	_usable_sectors = [];
	{
		if ( ( ( [ getmarkerpos _x , 1500 , WEST ] call F_getUnitsCount ) == 0 ) && ( ( [ getmarkerpos _x , 3500 , WEST ] call F_getUnitsCount ) > 0 ) ) then {
	...
	if ( count _usable_sectors > 0 ) then {
		...
		if ( random 100 < 33) then {
			...
			while { surfaceIsWater _spawnpos } do {
				_spawnpos = ( [ markerpos _spawnsector, random 100, random 360 ] call bis_fnc_relpos ) findEmptyPosition [10, 100, 'B_Heli_Transport_01_camo_F'];
		...
		{
			if ( (_patrol_startpos distance (markerpos _x) < 5000 ) && ( ( [ getmarkerpos _x , 4000 , WEST ] call F_getUnitsCount ) > 0 ) ) then {
		...
		waitUntil {
			sleep 30;
			( ( { alive _x } count ( units _grp ) ) == 0 ) || ( ( [ getpos leader _grp , 4000 , WEST ] call F_getUnitsCount ) == 0 )
		};
	};
