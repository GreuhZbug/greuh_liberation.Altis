...
while { true } do {
...

	{
		...
		if ( !(_next_truck in _managed_trucks) && (_truck_load > 0)) then {
				_action_id = _next_truck addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNLOAD_BOX" + "</t>","scripts\client\ammoboxes\do_unload_truck.sqf","",-500,true,true,"","build_confirmed == 0 && (_this distance _target < 5) && (vehicle player == player)"];
