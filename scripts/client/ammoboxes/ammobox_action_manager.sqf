...
while { true } do {
...

	{
		...
		if ( !(_next_truck in _managed_trucks) && (_truck_load > 0)) then {
				_action_id = _next_truck addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNLOAD_BOX" + "</t>","scripts\client\actions\do_unload_truck.sqf","",-500,true,true,"","build_confirmed == 0 && (_this distance _target < 5) && (vehicle player == player)"];
...

	{
		...
		if ( !(_next_truck in _checked_trucks)) then {
			...
			if ( _next_truck getVariable ["GRLIB_ammo_truck_load", 0] > 0 ) then {
				_next_truck removeAction ( _next_truck getVariable ["GRLIB_ammo_truck_action", -1] );
			};
		}
	...
	{
		_next_box = _x;
		if ( !(_next_box in _managed_boxes)) then {
			_action_id = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_LOAD_BOX" + "</t>","scripts\client\actions\do_load_box.sqf","",-501,true,true,"","build_confirmed == 0 && (_this distance _target < 5) && (vehicle player == player)"];
