...
private [ "_virtualItems", "_virtualWeapons", "_virtualMagazines", "_virtualBackpacks" ];
...
_fnc_linkedItems = {
        "
            if ( isText( _x >> 'Item' ) ) then {
                 _virtualItems pushback ( getText( _x >> 'Item' ) );
            };
        "configClasses _this;
    };
...
[ _virtualWeapons, _virtualMagazines, _virtualItems, _virtualBackpacks ]